  ...
  options = {
    programs.aerc = {
      ...
      queryMaps = mkOption {
        ...
        '';
        description = ''
          File containing notmuch query maps to use with notmuch.
        '';
      };
    };
  };

  # REVIEW https://github.com/nix-community/home-manager/pull/2894
  config = mkIf cfg.enable
    {
      ...
      home.file = {
        ...
        "${configDir}/aerc/querymaps.conf" = mkIf (cfg.queryMaps != [ ]) {
          source = pkgs.writeText "querymaps.conf" (strings.concatStringsSep "\n" cfg.queryMaps);
        };
      } // attrsets.mapAttrs'
        (name: value:
