...
#+begin_quote
  A condition in programming (also known in POSIX as EPIPE error code
  and SIGPIPE signal), when a process requests an output to pipe or
  socket, which was closed by peer

  #+begin_quote
    [[https://en.wikipedia.org/wiki/Broken_pipe][Wikipedia]]
  #+end_quote
#+end_quote
...
*** NixOS Configuration
   ...
   :END:
In order to boot correctly, you need to define some boot options[fn:1]:
...
[fn:1] https://nixos.wiki/wiki/NixOS_on_ARM/Raspberry_Pi_4#Configuration
